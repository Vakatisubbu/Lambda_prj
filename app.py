...
# AWS Config
S3_BUCKET = 'ccitprivatebucket'
S3_REGION = 'eu-west-2'

CLOUDFRONT_DOMAIN = 'd1vtlh2gjr59y.cloudfront.net'

# Lambda API URLs
SIGNUP_API_URL = "https://umqnffu3jl.execute-api.eu-west-2.amazonaws.com/Prod/Signup"
SIGNIN_API_URL = "https://umqnffu3jl.execute-api.eu-west-2.amazonaws.com/Prod/Signin"
WELCOME_API_URL = "https://umqnffu3jl.execute-api.eu-west-2.amazonaws.com/Prod/Welcome"
LIST_BOOKS_API_URL = "https://umqnffu3jl.execute-api.eu-west-2.amazonaws.com/Prod/ListBooks"
BARROW_BOOKS_API_URL = "https://umqnffu3jl.execute-api.eu-west-2.amazonaws.com/Prod/BarrowBooks"
RETURN_BOOKS_API_URL = "https://umqnffu3jl.execute-api.eu-west-2.amazonaws.com/Prod/ReturnBooks"
GET_BARROWED_BOOKS_API_URL = "https://umqnffu3jl.execute-api.eu-west-2.amazonaws.com/Prod/GetBarrowBooks"
GET_BARROWED_BOOKS_HISTORY_API_URL = "https://umqnffu3jl.execute-api.eu-west-2.amazonaws.com/Prod/GetBarrowHistory"
...
def welcome():
    #@app.route('/welcome')
    #def welcome():
       # user = session.get('user')  # Assuming user info is stored in session
       # image_url = None

        #if user and user.get("image"):
            # Construct full URL from S3 or CloudFront
         #   image_url = f"https://{CLOUDFRONT_DOMAIN}/{user['image']}"
    ...
    try:
        ...
        if response.status_code == 200:
            print("Existing user:", response.status_code)
            outer_data = response.json()
            body_data = outer_data.get("body")
            body_json = json.loads(body_data) if isinstance(body_data, str) else body_data
            user = body_json.get("user")  # <-- fix here
            image_url = f"https://{CLOUDFRONT_DOMAIN}/{user['image']}" if user and user.get("image") else None
            return render_template('welcome.html', user=user, image_url=image_url)
        else:
